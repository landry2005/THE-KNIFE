import gestione.GestoreRistoranti;
import gestione.GestoreUtenti;
import gestione.GestoreRecensioni;
...
public class TheKnife {
    ...
    private static void login() {
        ...
        if (gestoreUtenti.login(username, password)) {
            ...
            cittaCorrente = utente.getLuogoDomicilio();
    ...
    private static void visualizzaDettagliRistoranteCliente() {
        ...
        switch (scelta) {
            case 1:
                if (utente.isPreferito(idRistorante)) {
                    utente.rimuoviPreferito(idRistorante);
                    System.out.println("✓ Rimosso dai preferiti.");
                } else {
                    utente.aggiungiPreferito(idRistorante);
    ...
    private static void visualizzaPreferiti() {
        Utente utente = gestoreUtenti.getUtenteCorrente();
        List<String> preferiti = utente.getRistoratiPreferiti();
        ...
        int count = 1;
        for (String idRist : preferiti) {
            Ristorante r = gestoreRistoranti.cercaRistorantePerId(idRist);
            if (r != null) {
    ...
    private static void visualizzaMieRecensioni() {
        Utente utente = gestoreUtenti.getUtenteCorrente();
        List<Recenzione> recensioni = gestoreRecensioni.getRecensioniCliente(utente.getUsername());
     ...
     */
    private static void aggiungiRecensione(String idRistorante) {
        Utente utente = gestoreUtenti.getUtenteCorrente();
        
        if (gestoreRecensioni.hasRecensione(utente.getUsername(), idRistorante)) {
        ...
        if (gestoreRecensioni.aggiungiRecensione(utente.getUsername(), idRistorante, stelle, testo)) {
    ...
    private static void modificaRecensione() {
        ...
        if (gestoreRecensioni.modificaRecensione(utente.getUsername(), idRistorante, nuoveStelle, nuovoTesto)) {
    ...
    private static void eliminaRecensione() {
        ...
        if (conferma.equalsIgnoreCase("s")) {
            if (gestoreRecensioni.eliminaRecensione(utente.getUsername(), idRistorante)) {
